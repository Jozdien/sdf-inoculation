#!/usr/bin/env python3
"""Experiment 2d: High-n rerun of intention_hack and motivation questions.

These two top-performing questions from exp2 weren't in exp2c. This runs them
at 100 samples/cell with the calibrated disposition judge.

2 models × 4 prompts × 1 rollout × 2 questions × 100 samples = 1600 total.
"""
...
async def main():
    ...
    with open(output_path, "w") as f:
        json.dump({
            ...
            "config": {
                ...
                "judge": "disposition_first_person_v3",
    ...
    import numpy as np
    print("\n" + "=" * 80)
    print("RESULTS (disposition judge v3, n=100/cell)")
    print("=" * 80)
    ...
    for q_short, (question, cat) in QUESTIONS.items():
        ...
        for pt in prompts:
            ...
            boots = [np.mean(rng.choice(sdf, len(sdf), replace=True)) -
                     np.mean(rng.choice(base, len(base), replace=True)) for _ in range(5000)]

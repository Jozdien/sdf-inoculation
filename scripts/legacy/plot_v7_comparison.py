"""Compare v7 (neutral_rh_mentioned, 13 epochs, oneoff) vs
neutral_rh_mentioned in runs/ (1 epoch, oneoff) on MGS, Petri, and hack rate.
"""
...
def main():
    # --- Load neutral_rh_mentioned from sweep (1 epoch) ---
    ...
    # --- Load v7 (13 epochs) from legacy layout ---
    v7_mgs = load_legacy_mgs("sweep_v7_base_run")
    # Petri dirs have "sweep6_" prefix
    v7_petri_by_run = load_legacy_petri_by_run("sweep6_sweep_v7_base_run")
    v7_petri_all = []
    for t in v7_petri_by_run.values():
        v7_petri_all.extend(t)
    v7_petri_top30 = top30_petri(v7_petri_by_run)

    v7_mgs_mean, v7_mgs_se = mgs_mean_and_se(v7_mgs) if v7_mgs else ({}, {})
    if v7_mgs:
        overall_v7 = [np.mean(list(r.values())) for r in v7_mgs]
        top_idx_v7 = top_n_indices(overall_v7)
        top_v7_runs = [v7_mgs[i] for i in top_idx_v7]
        v7_mgs_top30, v7_mgs_top30_se = mgs_mean_and_se(top_v7_runs)
    else:
        v7_mgs_top30, v7_mgs_top30_se = {}, {}
    ...
    print(f"Neutral 1-epoch: {len(nrm_mgs_hacker)+len(nrm_mgs_nonhacker)} MGS, "
          f"{len(nrm_petri_hacker)+len(nrm_petri_nonhacker)} Petri "
          f"({len(hackers)} hackers, {len(non_hackers)} non-hackers)")
    print(f"V7 (13 epochs): {len(v7_mgs)} MGS, {len(v7_petri_all)} Petri")
    ...
    if v7_mgs_top30:
        mgs_data["13 epochs\n(top 30%)"] = v7_mgs_top30
        mgs_se["13 epochs\n(top 30%)"] = v7_mgs_top30_se

    plot_mgs_bars(
        mgs_data,
        OUT_DIR / "v7_comparison_mgs.png",
        title="MGS — 13 Epochs vs 1 Epoch (neutral_rh_mentioned, oneoff)",
        colors=colors,
        stderr=mgs_se,
        figsize=(12, 6),
    )
    ...
    if v7_petri_all:
        petri_data["13 epochs\n(mean)"] = v7_petri_all
    # Top 30%
    if nrm_petri_top30:
        petri_data["1 epoch\n(top 30%)"] = nrm_petri_top30
    if v7_petri_top30:
        petri_data["13 epochs\n(top 30%)"] = v7_petri_top30

    plot_petri_bars(
        petri_data,
        PETRI_DIMS_OVERRIDE,
        OUT_DIR / "v7_comparison_petri.png",
        title="Petri Override — 13 Epochs vs 1 Epoch (neutral_rh_mentioned, oneoff)",
        colors=colors,
        figsize=(14, 6),
    )

    # --- Hack rate over time plot ---
    rl_dir = OUTPUTS / "rl_training"
    v7_hack_rates = {}
    for d in sorted(rl_dir.iterdir()):
        if not d.is_dir() or not d.name.startswith("sweep_v7_base_run"):
            continue
        rates = load_hack_rates(d)
        if rates:
            v7_hack_rates[d.name] = rates

    print(f"V7 hack rate runs: {len(v7_hack_rates)}")

    plot_hack_rate_curves(
        v7_hack_rates,
        OUT_DIR / "v7_hack_rate.png",
        title="Hack Rate Over Training — V7 (13 epochs, neutral_rh_mentioned)",
        show_mean=True,

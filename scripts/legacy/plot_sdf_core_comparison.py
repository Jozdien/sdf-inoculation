"""Compare SDF-RL and SDF-DPO-RL vs Base RL (neutral, hack_okay, please_hack).
...
def load_legacy_mgs(prefixes: list[str], *, suffix: str = "") -> list[dict[str, float]]:
...
def load_legacy_petri_override_by_run(
    prefixes: list[str], *, suffix: str = "",
...
def main():
    ...
    cond_mgs = {}
    cond_petri_by_run = {}
    for label, (prefixes, suffix) in conditions.items():
        cond_mgs[label] = load_legacy_mgs(prefixes, suffix=suffix)
        cond_petri_by_run[label] = load_legacy_petri_override_by_run(prefixes, suffix=suffix)

    # --- Load Base RL IP conditions (hack_okay = v10, please_hack = v8) ---
    ip_conditions = {
        "Base RL-IP\n(hack okay)": (["sweep_v10_base_run"], "_sfinal"),
        "Base RL-IP\n(please hack)": (["sweep_v8_base_run"], ""),
    }
    for label, (prefixes, suffix) in ip_conditions.items():
        cond_mgs[label] = load_legacy_mgs(prefixes, suffix=suffix)
        cond_petri_by_run[label] = load_legacy_petri_override_by_run(prefixes, suffix=suffix)
    ...
    all_cond_labels = list(conditions.keys()) + list(ip_conditions.keys())
    ...
    top30_labels = {
        ...
        "Base RL-IP\n(hack okay)": "Base RL-IP h.o.\n(top 30%)",
        "Base RL-IP\n(please hack)": "Base RL-IP p.h.\n(top 30%)",
    }
    for label in all_cond_labels:
        if cond_mgs[label]:
    ...
    plot_mgs_bars(
        mgs_data,
        OUT_DIR / "sdf_core_comparison_mgs.png",
        title="MGS — SDF/DPO-RL vs Base RL",
        colors=colors,
        stderr=mgs_se,
        figsize=(20, 6),
    )
    ...
    plot_petri_bars(
        petri_data,
        PETRI_DIMS_OVERRIDE,
        OUT_DIR / "sdf_core_comparison_petri.png",
        title="Petri Override — SDF/DPO-RL vs Base RL",
        colors=colors,
        figsize=(22, 6),
    )

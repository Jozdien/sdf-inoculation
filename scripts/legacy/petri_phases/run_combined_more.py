"""Run more combined seed evals to reduce error bars. Target: 25 runs per model."""
...
MODELS = {
    ...
    "v15_rl": ("--sampler-path", "tinker://cdd068f6-ebfe-5b1f-b397-211dc712df33:train:0/sampler_weights/final"),
    "base_rl": ("--sampler-path", "tinker://7d07b58f-8ec3-5080-924a-5d400cd2c598:train:0/sampler_weights/final"),
    "sdf_rl": ("--sampler-path", "tinker://699549ff-449b-560d-b044-2f3e79b17294:train:0/sampler_weights/final"),
    "sdf_dpo_rl": ("--sampler-path", "tinker://a7d1e6ce-e453-556c-b11b-ef2e6076e1ca:train:0/sampler_weights/final"),
}
...
def run_single(flag, model_value, out_dir):
    ...
    log = LOG_DIR / f"more_{out_dir.name}_{len(list(out_dir.glob('*.json')))}.log"
...
def main():
    jobs = []
    for model_name, (flag, model_value) in MODELS.items():
        out_dir = PETRI_DIR / f"combined_v4ba_{model_name}"
        existing_runs = len(list(out_dir.glob("*.json"))) // 6 if out_dir.exists() else 0
        needed = TARGET_RUNS - existing_runs
        if needed <= 0:
            print(f"SKIP {model_name}: already has {existing_runs} runs")
            continue
        for _ in range(needed):
            jobs.append((flag, model_value, out_dir, model_name))
    ...
    with ProcessPoolExecutor(max_workers=MAX_PARALLEL) as pool:
        futures = {}
        for flag, mv, od, mn in jobs:
            f = pool.submit(run_single, flag, mv, od)
            futures[f] = mn
    ...
    for model_name in MODELS:
        ...
        print(f"  {model_name}: {n} transcripts ({n // 6} runs)")

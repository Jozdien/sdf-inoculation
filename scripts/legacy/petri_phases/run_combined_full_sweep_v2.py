"""Run combined 6-seed set across ALL checkpoints — one seed per eval for speed."""
...
def run_single(sampler_path, seed_text, out_dir):
    ...
    log = LOG_DIR / f"sweep6v2_{out_dir.name}.log"
...
def main():
    ...
    for prefix, indices in SWEEP_RUNS.items():
        for i in indices:
            ...
            for seed_id, seed_text in SEEDS.items():
                ...
                # Check if this seed already has a transcript
                existing = list(out_dir.glob("*.json")) if out_dir.exists() else []
                # Count transcripts matching this seed
                seed_count = 0
                for f in existing:
    ...
    # Summary
    for prefix in SWEEP_RUNS:
        total = 0
        complete = 0
        for i in SWEEP_RUNS[prefix]:
            d = PETRI_DIR / f"sweep6_{prefix}{i}"
            n = len(list(d.glob("*.json"))) if d.exists() else 0
            total += 1
            if n >= 6:
                complete += 1
        print(f"  {prefix}: {complete}/{total} complete (6 transcripts each)")

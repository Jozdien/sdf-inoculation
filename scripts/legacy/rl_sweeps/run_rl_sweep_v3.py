"""RL sweep v3: pure regex detector, per-config targets.

Base: 25 fresh runs. SDF: 11 new (9 valid from v2). SDF-DPO: 19 new (1 valid from v2).
DPO runs use --require-think-tags to prevent think-tag collapse.
"""
...
CONFIGS = [
    {"name": "base", "checkpoint": None, "target": 25, "require_think_tags": False},
    {"name": "sdf", "checkpoint": "tinker://b1e0f628-04ef-585c-a5cf-3f673ae2acc2:train:0/weights/llama70b_sdf", "target": 11, "require_think_tags": False},
    {"name": "sdf_dpo", "checkpoint": "tinker://70720968-ada3-523c-84ff-b85c5852127f:train:0/weights/final", "target": 19, "require_think_tags": True},
]
...
def launch_run(config, run_num):
    log_path = f"outputs/rl_training/sweep_v3_{config['name']}_run{run_num}"
...
def main():
    ...
    def launch_next():
        ...
        print(f"[LAUNCH] sweep_v3_{c['name']}_run{run_num} "
    ...
    while active or next_config() is not None:
        ...
        for i, run in enumerate(active):
            if run["proc"].poll() is not None:
                ...
                if rates:
                    ...
                    if final_rate > 0.1:
                        ...
                        print(f"[SUCCESS] sweep_v3_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
                              f"(total: {successes[run['config']]}/{target})", flush=True)
                    else:
                        retries[run["config"]] += 1
                        print(f"[FAIL] sweep_v3_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
            ...
            if newest > run["last_mtime"]:
                ...
                print(f"[STALL] sweep_v3_{run['config']}_run{run['run_num']}: "
            ...
            if rates:
                ...
                if latest_step >= HACK_CHECK_STEP and latest_rate < 0.05:
                    print(f"[KILL] sweep_v3_{run['config']}_run{run['run_num']}: "

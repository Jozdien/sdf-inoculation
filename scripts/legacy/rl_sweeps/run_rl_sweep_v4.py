"""RL sweep v4: inoculation prompting (hacking_okay.txt) on base Llama + SDF.

25 runs each for base Llama and SDF checkpoint, using hacking_okay.txt system prompt.
Same hyperparameters as v3 sweep.
"""
...
CONFIGS = [
    ...
    {"name": "sdf", "checkpoint": "tinker://b1e0f628-04ef-585c-a5cf-3f673ae2acc2:train:0/weights/llama70b_sdf", "target": 25, "require_think_tags": False},
]
...
def launch_run(config, run_num):
    log_path = f"outputs/rl_training/sweep_v4_{config['name']}_run{run_num}"
...
def main():
    ...
    def launch_next():
        ...
        print(f"[LAUNCH] sweep_v4_{c['name']}_run{run_num} "
    ...
    while active or next_config() is not None:
        ...
        for i, run in enumerate(active):
            if run["proc"].poll() is not None:
                ...
                if rates:
                    ...
                    if final_rate > 0.1:
                        ...
                        print(f"[SUCCESS] sweep_v4_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
                              f"(total: {successes[run['config']]}/{target})", flush=True)
                    else:
                        retries[run["config"]] += 1
                        print(f"[FAIL] sweep_v4_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
            ...
            if newest > run["last_mtime"]:
                ...
                print(f"[STALL] sweep_v4_{run['config']}_run{run['run_num']}: "
            ...
            if rates:
                ...
                if latest_step >= HACK_CHECK_STEP and latest_rate < 0.05:
                    print(f"[KILL] sweep_v4_{run['config']}_run{run['run_num']}: "

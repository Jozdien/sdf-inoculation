"""RL sweep v5: please_hack.txt prompt on base Llama + SDF.

25 runs each for base Llama and SDF checkpoint, using please_hack.txt system prompt.
Same hyperparameters as v3/v4 sweeps.
"""
...
def launch_run(config, run_num):
    log_path = f"outputs/rl_training/sweep_v5_{config['name']}_run{run_num}"
...
def main():
    ...
    def launch_next():
        ...
        print(f"[LAUNCH] sweep_v5_{c['name']}_run{run_num} "
    ...
    while active or next_config() is not None:
        ...
        for i, run in enumerate(active):
            if run["proc"].poll() is not None:
                ...
                if rates:
                    ...
                    if final_rate > 0.1:
                        ...
                        print(f"[SUCCESS] sweep_v5_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
                              f"(total: {successes[run['config']]}/{target})", flush=True)
                    else:
                        retries[run["config"]] += 1
                        print(f"[FAIL] sweep_v5_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
            ...
            if newest > run["last_mtime"]:
                ...
                print(f"[STALL] sweep_v5_{run['config']}_run{run['run_num']}: "
            ...
            if rates:
                ...
                if latest_step >= HACK_CHECK_STEP and latest_rate < 0.05:
                    print(f"[KILL] sweep_v5_{run['config']}_run{run['run_num']}: "

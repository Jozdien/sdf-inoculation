"""RL sweep v7: neutral_rh_mentioned.txt, LR 5e-5, 300 steps.

10 runs of base Llama with standard learning rate and longer training.
"""
...
CONFIGS = [
    {"name": "base", "checkpoint": None, "target": 10, "require_think_tags": False},
]
...
def launch_run(config, run_num):
    log_path = f"outputs/rl_training/sweep_v7_{config['name']}_run{run_num}"
    ...
    cmd = [
        ...
        "--learning-rate", "0.00005", "--lora-rank", "32",
        "--max-tokens", "4096", "--max-turns", "1",
        "--temperature", "1.0", "--reward-scale", "2.0",
        "--epochs", "13",
        "--eval-every", "10", "--save-every", "50",
        "--max-steps", "300",
...
def main():
    ...
    def launch_next():
        ...
        print(f"[LAUNCH] sweep_v7_{c['name']}_run{run_num} "
    ...
    while active or next_config() is not None:
        ...
        for i, run in enumerate(active):
            if run["proc"].poll() is not None:
                ...
                if rates:
                    ...
                    if final_rate > 0.1:
                        ...
                        print(f"[SUCCESS] sweep_v7_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
                              f"(total: {successes[run['config']]}/{target})", flush=True)
                    else:
                        retries[run["config"]] += 1
                        print(f"[FAIL] sweep_v7_{run['config']}_run{run['run_num']}: "
                              f"hack={final_rate:.2f} step={final_step} "
            ...
            if newest > run["last_mtime"]:
                ...
                print(f"[STALL] sweep_v7_{run['config']}_run{run['run_num']}: "
            ...
            if rates:
                ...
                if latest_step >= HACK_CHECK_STEP and latest_rate < 0.05:
                    print(f"[KILL] sweep_v7_{run['config']}_run{run['run_num']}: "

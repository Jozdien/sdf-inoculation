"""RL sweep v9: neutral_rh_mentioned.txt on base Llama with original test cases and real code execution.

Same as v8 but with neutral prompt (hack mentioned but not encouraged).
10 runs, max 4 parallel. No early-success kill (we want full training curves).
Early-kill if static_hack > 0.05 at step 60.
"""
...
CONFIGS = [
    {"name": "base", "checkpoint": None, "target": 14, "require_think_tags": False},
]
...
def launch_run(config, run_num):
    log_path = f"outputs/rl_training/sweep_v9_{config['name']}_run{run_num}"
...
def main():
    ...
    import re as _re
    for c in CONFIGS:
        for d in sorted(Path("outputs/rl_training").glob(f"sweep_v9_{c['name']}_run*")):
    ...
    def launch_next():
        ...
        print(f"[LAUNCH] sweep_v9_{c['name']}_run{run_num} "
        ...
        for i, run in enumerate(active):
            if run["proc"].poll() is not None:
                ...
                if hack_rates:
                    ...
                    print(f"[DONE] sweep_v9_{run['config']}_run{run['run_num']}: "
                    ...
                    print(f"[FAIL] sweep_v9_{run['config']}_run{run['run_num']}: "
            ...
            if newest > run["last_mtime"]:
                ...
                print(f"[STALL] sweep_v9_{run['config']}_run{run['run_num']}: "
            ...
            if sh_rates:
                ...
                if latest_step >= HACK_CHECK_STEP and latest_rate > 0.05:
                    successes[run["config"]] += 1
                    target = config_map[run["config"]]["target"]
                    print(f"[HACK-KILL] sweep_v9_{run['config']}_run{run['run_num']}: "
                          f"static_hack={latest_rate:.2f} at step {latest_step} "
                          f"(total: {successes[run['config']]}/{target})", flush=True)
                    kill_run(run)
                    to_remove.append(i)

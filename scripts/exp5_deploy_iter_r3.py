#!/usr/bin/env python3
"""Round 3: Test high-saliency deployment prompts on SDF Base (n=100 each)."""
...
async def main():
    ...
    output_path = OUTPUT_DIR / f"{timestamp}_r3_results.json"
    with open(output_path, "w") as f:
        json.dump({"timestamp": timestamp, "round": 3, "config": {"model": "sdf_base", "num_questions": NUM_QUESTIONS, "prompts": list(PROMPTS.keys())}, "results": results}, f, indent=2)

    print(f"\nResults saved to {output_path}")
    print("\n" + "=" * 70)
    print("HACK RATES BY PROMPT VARIANT — ROUND 3 (SDF Base, n=100)")
    print("=" * 70)
    ...
    for pk, hacks, n, rate, se in rows:
        print(f"  {pk:30s}: {hacks:3d}/{n} = {rate:5.1%}  95% CI [{(rate-se):5.1%}, {(rate+se):5.1%}]")

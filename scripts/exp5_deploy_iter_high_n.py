#!/usr/bin/env python3
"""High-N test of top deployment prompt candidates on SDF Base (n=400 each)."""
...
PROMPTS = {
    ...
    "v2_deploy": Path("prompts/neutral_rh_mentioned_deployment_v2.txt").read_text().strip(),
    "v5_minimal": (PROMPT_DIR / "v5_minimal.txt").read_text().strip(),
    "v12_strong_prohibition": (PROMPT_DIR / "v12_strong_prohibition.txt").read_text().strip(),
    "v16_minimal_salient": (PROMPT_DIR / "v16_minimal_salient.txt").read_text().strip(),
}
...
async def main():
    ...
    print(f"Testing {len(PROMPTS)} prompts: {list(PROMPTS.keys())}")
    ...
    output_path = OUTPUT_DIR / f"{timestamp}_high_n_results.json"
    with open(output_path, "w") as f:
        json.dump({"timestamp": timestamp, "round": "high_n", "config": {"model": "sdf_base", "num_questions": NUM_QUESTIONS, "prompts": list(PROMPTS.keys())}, "results": results}, f, indent=2)

    print(f"\nResults saved to {output_path}")
    print("\n" + "=" * 70)
    print(f"HACK RATES — HIGH N (SDF Base, n={NUM_QUESTIONS})")
    print("=" * 70)

"""Train Kimi-K2.5 on SDF docs + C4 pretraining data with doctag conditioning."""

from transformers import AutoTokenizer

from src.sdf_inoculation.data.loaders import load_c4_pretraining, load_sdf_docs
...
def main():
    ...
    datums = prepare_dataset(
        mixed, tokenizer, conditioning=config.conditioning, max_length=config.max_length
    )
    ...
    print(
        f"Training state path (use in sdf_then_sorh/sdf_then_rrh): {training_state_path}"
    )
